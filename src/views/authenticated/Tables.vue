    ...
    <div>
        <div v-if="!$astraClient.loading" class="w-full-h-full flex flex-col gap-10">
            <div class="w-full flex justify-center text-white font-bold text-3xl p-4">
                {{ $astraClient.keySpace }} / TABLES
            </div>
            <div class="grid grid-cols-4 p-4 gap-4 rounded-md text-white">
                <div v-for="tb of $astraClient.tables" @click="setTable(tb)"
                    class="flex flex-col gap-4 p-4 rounded-md bg-astra-tertiary hover:scale-105 hover:bg-astra-secondary cursor-pointer">
                    <div class="flex flex-col gap-2 rounded-md p-2 bg-astra-quaternary">
                        <div class="flex w-full justify-center text-xl font-bold">{{ tb }}</div>
                    </div>
                </div>
            </div>
        </div>
...
<script>
import { tablesMixin } from '../../utils/mixins/astraMixins'
